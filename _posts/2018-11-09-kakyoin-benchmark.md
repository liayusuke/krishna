...
这里明显看到了`recvfrom/sendto`基本对等，并且数量等于2K * 10，但是`epoll_Wait`竟然差了20倍之多！
...
为了验证，我在goclient的每个routine后，额外`sleep rand.Intn(1000)` 微秒，再次压测，cpu升到5%，同时strace里`epoll_Wait`变成10K+...
...
> + 后来回想整个过程，其实现象也很明显，只是查问题的思路不太对。golang真的是方便鸭，不过能有这样的时间查问题，感觉以前不太可能的，业务需求都搞不完
>
> + 埋头搬砖的同时，也要抬头看看天，只是不知道广州的天气，现在如何了，哈
